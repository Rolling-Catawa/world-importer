...
public class WIConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        autoThrottleBtn = new Button(cx + 78, y, 72, 20,
        ...
        this.addRenderableWidget(new Button(cx - 152, y, 100, 20, Component.literal("开始上传"), btn -> {
            applyConfig();
        ...
        this.addRenderableWidget(new Button(cx - 48, y, 100, 20, Component.literal("取消上传"), btn -> {
            ClientUploadManager.cancel();
            ClientImportState.reset();
        }));

        this.addRenderableWidget(new Button(cx + 56, y, 96, 20, Component.literal("保存关闭"), btn -> {
            applyConfig();
            WIConfig.save();
            this.onClose();
        }));
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        int cx = this.width / 2;
        drawString(poseStack, this.font, "Region路径:", cx - 150, 25, 0xAAAAAA);
        drawString(poseStack, this.font, "间隔(t)", cx - 150, 53, 0xAAAAAA);
        drawString(poseStack, this.font, "包(KB)", cx - 70, 53, 0xAAAAAA);
        drawString(poseStack, this.font, "光照/t", cx + 10, 53, 0xAAAAAA);
        ...
        if (!"idle".equals(status) && !"done".equals(status)) {
            ...
            drawCenteredString(poseStack, this.font, info, cx, this.height - 38, 0xFFFFFF);

            int tpsColor = tps >= 18 ? 0x55FF55 : tps >= 15 ? 0xFFFF55 : 0xFF5555;
            drawCenteredString(poseStack, this.font, String.format("TPS: %.1f", tps), cx, this.height - 26, tpsColor);

            int bw = 300, bx = cx - bw / 2, by = this.height - 14;
            fill(poseStack, bx, by, bx + bw, by + 6, 0xFF333333);
            fill(poseStack, bx, by, bx + (total > 0 ? bw * Math.min(completed, total) / total : 0), by + 6, 0xFF55FF55);
        }
